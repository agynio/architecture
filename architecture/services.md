...
|---------|------------|------------------|-------|------|
| Agents | [agynio/agents](https://github.com/agynio/agents) | [Agents Service](agents-service.md) | Control plane | Manages agent resource definitions, environments and their contents (volumes, MCP servers, init scripts, ENVs), sandboxes, skills, and related configuration. |
...
| Platform Controller | [agynio/platform-controller](https://github.com/agynio/platform-controller) | [Platform Resource Provisioning](operations/platform-provisioning.md) | Control plane | Reconciles the resources a release declares the platform should contain — the system organization, cluster administrators, shipped images, the runner, bundled apps and the overlay's baseline policies — against the ordinary Gateway API. |
