...
| **Reference Checks** | `ResolveAgentExists(agent_id, organization_id)` and `ResolveEnvironmentExists(environment_id, organization_id)` — verify the resource exists in that organization, used by another service to validate a reference before it is persisted. Internal only, via Istio, no OpenFGA check. Modeled on [`Secrets.ResolveSecretExists`](secrets.md#responsibilities); the caller for both today is [Apps](apps-service.md#reference-validation) resolving an `x-agyn-ref` property | Resolve |
