...
| **Private-resource reference lookup** | Provide `CountRulesReferencingPrivateResource`, `ListMediatedPrivateResources`, and `ListAttachedRuleDomains` for the Networks service's referential-integrity guards, mediation reconciliation, and collision fast-fail |
| **Intercept collision detection** | Each reconciliation pass, report identities authorized for two services with overlapping interceptions. No write-time check can prevent that state — see [Private Networks — Hostname collisions](private-networks.md#hostname-collisions) |
...
| **ListAttachedRuleDomains** | **Internal-only.** Given an `agent` or `environment` principal, return the `domain_pattern` and `ports` of every public-target rule reaching it — attached to it directly, to a group it belongs to, or to an agent's environment. Called by the Networks service to fast-fail a [hostname collision](private-networks.md#hostname-collisions) before granting that principal a resource |

### Egress Rule Attachment CRUD

| Method | Description |
|---|---|
| **CreateEgressRuleAttachment** | Attach a rule to an agent or an environment (exactly one target). Validates: the target belongs to the rule's organization (via an [Authorization](authz.md) check), attachment is unique on `(rule_id, target)`, and — for a public-target rule, best-effort — that the target cannot already dial a PrivateResource whose `intercept_host` collides with the rule's `domain_pattern` (via `Networks.ListPrivateResourcesReachableBy`, expanded through the target's groups and an agent's environment). The collision check is a fast-fail, not an invariant: writes in [Groups](groups-service.md) and [Agents](agents-service.md) reach the same state without passing through here — see [Private Networks — Hostname collisions](private-networks.md#hostname-collisions). Creates the per-attachment Dial policy via Ziti Management |
...
6. **Intercept collisions per identity.** Expand the role attributes named by this service's attachment policies and by the resource Dial policies reported by `Networks.ListPrivateResourcesReachableBy` — through group membership and through environment-to-workload — and report any identity authorized for two services whose `intercept.v1` address and port overlap. Reported, never repaired: see [Private Networks — Hostname collisions](private-networks.md#hostname-collisions).

**Reconciliation repairs; it does not record.** Every step above either converges OpenZiti to the rule table or reports. `EgressRule` and `EgressRuleAttachment` carry no `provisioning_state` — unlike the [Networks service](networks-service.md) resources, whose failed provisioning must survive across passes to be retried, a rule's OpenZiti materialization is re-derivable from the row on every pass, so there is nothing a status field would hold that the next pass would not recompute. Conditions the reconciler cannot repair (step 2's missing resource, step 6's collisions) surface as logs, metrics, and Console warnings on the rule.
