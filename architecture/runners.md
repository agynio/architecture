...
| `persistent_shells` | bool | Whether shells in this workload outlive the connections that reach them. Resolved from the environment's [`persistent_shells`](resource-definitions.md#environment) by the [Orchestrator](agents-orchestrator.md#workload-spec-assembly) at start and fixed for the workload's life. Read by the [Terminal Proxy](terminal-proxy.md#persistent-shells) on the `GetWorkload` it already makes to resolve the runner — which is why it lives here rather than being looked up per ticket, and why editing the environment does not reconfigure a workload someone is working in |
| `created_at` | timestamp | Creation time |
