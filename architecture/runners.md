...
| **ListWorkloads** | List workloads for an organization. Used by the Console monitoring dashboard |
...
Workload query methods (`ListWorkloads`, `ListWorkloadsByThread`, `GetWorkload`) are used by the UI. The Console calls `ListWorkloads` to populate the monitoring dashboard with active workloads for an organization. The Chat UI calls `ListWorkloadsByThread` to populate the container popover in the conversation header. It calls `GetWorkload` to get container details before opening a terminal session.
