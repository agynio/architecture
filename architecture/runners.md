...
| `instance_id` | string (nullable) | Runner-assigned **runtime** identifier (Pod name) returned by `StartWorkload`. NULL until `StartWorkload` completes. Unrelated to `agent_instance_id` |
| `runner_id` | string (UUID) | Runner hosting this workload |
| `agent_instance_id` | string (UUID) | [Agent instance](agent-instances.md) this workload serves. The workload processes this instance's inbox |
| `agent_id` | string (UUID) | Agent class the instance was spawned from (denormalized from the instance for filtering and display) |
...
Tracks persistent volumes actually provisioned on runners. Each record represents one provisioned instance of an Agents service [Volume](resource-definitions.md#volume) for a specific [agent instance](agent-instances.md) — one Volume definition can have many provisioned instances, one per agent instance.
...
| `instance_id` | string (nullable) | Runner-assigned **runtime** identifier (PVC name). NULL until the reconciliation loop confirms the PVC exists on the runner. Unrelated to `agent_instance_id` |
| `volume_id` | string (UUID) | ID of the [Volume](resource-definitions.md#volume) in the Agents service. Together with `agent_instance_id`, uniquely identifies the provisioned instance |
| `agent_instance_id` | string (UUID) | Agent instance this volume serves |
| `runner_id` | string (UUID) | Runner on which the volume is provisioned |
| `agent_id` | string (UUID) | Agent class that owns the volume definition (denormalized) |
...
Workloads and provisioned volumes are associated to [agent instances](agent-instances.md) through their `agent_instance_id` fields. Runners owns those runtime records; the Agents service owns the instance entity and its inbox; Threads owns messages and participants.
