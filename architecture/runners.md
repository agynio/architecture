...
Tracks persistent volumes actually provisioned on runners. For agent-owned volumes, each record represents one provisioned instance of an Agents service [Volume](resource-definitions.md#volume) for a specific [agent instance](agent-instances.md) — one Volume definition can have many provisioned instances, one per agent instance. Sandbox workspace volumes are **runtime-only**: provisioned directly by the Orchestrator with no Agents service Volume definition behind them (`volume_id` NULL), and their lifecycle is bound to the sandbox.
