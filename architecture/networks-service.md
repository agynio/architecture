...
| **ListPrivateResourcesReachableBy** | **Internal-only.** Given an `agent` or `environment` principal, return the resources it can dial, with their `intercept_host` and `intercept_ports` — expanding grants made to the principal's groups and, for an agent, to its environment. Called by the [EgressRules service](egress-rules-service.md) to fast-fail a [hostname collision](private-networks.md#hostname-collisions) on attach, and each reconciliation pass to detect the collisions no write-time check catches |

### PrivateResourceAccess CRUD

| Method | Description |
|---|---|
| **CreatePrivateResourceAccess** | Grant access. Validates: principal exists (for `agent` / `user` / `app` via `Identity.GetIdentityType` — also confirms `principal_type` matches the registered identity type; for `group` via `Groups.GetGroup`; for `environment` via `Agents.GetEnvironment`); the principal belongs to the resource's organization (cross-org `Check` via [Authorization](authz.md) `org` relation on `<principal_type>:<principal_id>`, which the [`environment` type](authz.md#environment) carries like every other principal); the grant is unique on `(resource_id, principal_type, principal_id)`; and — for `agent` and `environment` principals, best-effort — that no public egress rule colliding with the resource's `intercept_host` is attached to that principal, its groups, or an agent's environment (via `EgressRules.ListAttachedRuleDomains`). That check is a fast-fail rather than an invariant; group and environment writes reach the same state without passing through here, so [detection](private-networks.md#hostname-collisions) is what holds. Creates the per-grant Dial policy |
