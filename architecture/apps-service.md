...
The Apps Service manages app definitions and installations — the configuration entities that define [apps](apps.md), their profiles, their visibility, and their per-organization installations. It handles both control plane operations (definition, installation) and data plane operations (profile resolution, slug lookup on the Gateway request path).
...
| **GetAppBySlug** | Get an app definition by owning organization ID + slug |
...
| **ListInstallations** | List installations. Supports filtering by organization and by app definition |
...
| `app_id` | string (UUID) | Reference to the app definition |
...
1. Org admin calls `InstallApp` with the app definition ID, target organization, slug, and configuration.
...
PostgreSQL. Apps Service owns its database — `app_definitions` and `app_installations` tables.
