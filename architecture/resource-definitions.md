...
| `persistent_shells` | bool | `true` | Whether shells in this environment's workloads outlive the connections that reach them — see [Terminal Proxy — Persistent Shells](terminal-proxy.md#persistent-shells). Resolved by the [Orchestrator](agents-orchestrator.md#workload-spec-assembly) at each workload start and recorded on the [workload](runners.md#workload-resource), so an edit applies to the next start rather than reconfiguring a sandbox someone is working in. Off costs a multiplexer's memory per workload and gives back the behavior of a dropped SSH connection |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `shell_id` | string | | Opaque, client-generated, `^[A-Za-z0-9_-]{1,64}$`. Passed verbatim as `shell_id` on a terminal session — see [Persistent Shells](terminal-proxy.md#persistent-shells) |
...
| `openziti_upstream_service_ids` | map<uint16, string> | | OpenZiti service ID of `private-<id>-upstream-<intercept_port>` per entry in `intercept_ports` — the tunnel-bound services the Egress Gateway dials, each hard-coding the paired target port. Present only while `mediation` is `egress_gateway`. Internal — not returned through the Gateway |
...
Reserved `intercept_host` patterns rejected at create time: `*.agyn`, `*.svc`, `*.cluster.local`, anything overlapping `100.64.0.0/10` (OpenZiti synthetic CIDR), `localhost`, `127.0.0.0/8`, and `::1/128`. An `intercept_host` equal to some [Egress Rule](#egress-rule)'s `domain_pattern` is allowed here and refused later, at the point where one principal would come to hold both — see [Private Networks — Hostname collisions](private-networks.md#hostname-collisions).
