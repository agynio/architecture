...
| `agyn local upgrade [--resume]` | Upgrade the `agyn-platform` and `agyn-apps` releases in the running VM to the newest charts, keeping the VM and its data — see [Upgrade](#upgrade). `--resume` continues an upgrade that was interrupted. Everything from the image (k3s, Istio, cert-manager, OpenZiti) moves only by recreating the VM — see [Upgrade Model](operations/local-bundle.md#upgrade-model) |
...
**The host's port is written into the release, not over its output.** The port belongs in the release's values — it is already there, as the service URLs and `keycloak.wiring.ingressHostPort.port` — but nothing wrote the host's choice into them, so an upgrade re-applied the port the image was baked with and a second pass patched the rendered workloads back. Every upgrade therefore rolled most of the platform twice, with a window in between where services dialled a port that published nothing and backed off to five-minute retries.

`upgrade` derives an overlay from the release's own values, replacing the port they name with the host's, and Helm renders it correctly the first time. Derived rather than enumerated: the count of keys carrying it is a chart detail, and one added upstream would otherwise be missed in silence. Only the exact stale number is replaced, and a release whose values already agree produces no overlay — which is every VM on the default port. Values naming a port the VM does not serve are themselves a reason to upgrade, chart version or not.
...
**Recovery goes forward.** Helm's own remedy is a rollback to the last deployed revision, and it is the wrong direction here: an interruption leaves part of the cluster on the new chart already, and rolling those workloads back moves them by however many versions the upgrade spanned. Platform migrations are forward-only and guarantee [one version of backward compatibility](operations/database-migrations.md#5-backward-compatibility-window) — a VM upgraded after sitting for a while spans far more than one. `--resume` therefore drops the in-flight revision, which touches no workload, and then upgrades to the target: the direction the data has already gone.
