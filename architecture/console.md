...
| `ThreadsGateway` | `ListOrganizationThreads`, `GetMessages` | Org owner (`can_view_threads`) | Threads |
...
| `RunnersGateway` | `RegisterRunner`, `GetRunner`, `ListRunners`, `UpdateRunner`, `DeleteRunner`, `ListWorkloads`, `GetWorkload`, `StreamWorkloadLogs` | Cluster-scoped: cluster admin. Org-scoped: org owner (workload read + logs: org member) | Runners, Workloads (workload list, detail, and container logs) |
