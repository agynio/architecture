...
13. **LLM mode** (from the environment + LLM service): when the environment's [`llm_mode`](resource-definitions.md#environment) is `native`, the orchestrator asks the [LLM service](llm.md#subscription-management) which vendors have a [Subscription](providers.md#subscription) attached at the agent or environment scope, and adds an `llm-native-<vendor>` role attribute to the workload's OpenZiti identity for each. Those attributes are the only thing that opts a workload into [vendor interception](llm-proxy.md#vendor-intercept-services); no OpenZiti service or policy is created per workload, per environment, or per attachment. In `platform` mode no attribute is added and vendor traffic is not intercepted.
