...
- Marks the turn's thread messages read via `Threads.AckMessages(message_ids)`, at the same point and for the same items — every item of `source_kind = thread` carries the `message_id` to pass. This is a separate call with a separate meaning: the inbox ack says the runtime processed the item, the message ack says the agent was shown it. `direct` items have no message and are not marked. The call is idempotent, so messages a shell already fetched with [`agyn threads read --unread`](agyn-cli.md#unread-and-acknowledgment) during the turn cost nothing to re-mark.
...
| **Trace hook** | Opens the trace for this wake cycle, and arranges for the agent CLI's turns to be recorded in it. For a third-party CLI that means registering the platform's [trace hook](tracing.md#the-trace-hook) on turn completion, in the CLI's own config — `config.toml` for Codex, `~/.claude/settings.json` for Claude Code — the same files this step already writes; the hook ships with `agynd`, is told which transcript format to expect and which trace to write into, and after each turn reads the CLI's session transcript and exports the turn to [Tracing](tracing.md) at `tracing.agyn`. For [agn](agn-cli.md), which [exports its own spans](tracing.md#a-cli-that-exports-its-own-spans), it is the trace id that is handed over instead — the endpoint reaches `agn` from the workload's environment. Either way the prompt, the model's reply, and each tool call's input and output are recorded in the trace this cycle opened |
...
Codex is the case today. A subscription-mode Codex CLI reads `~/.codex/auth.json` before it reaches the network and refuses to start without one, so `agynd` writes a document in the shape Codex expects — `auth_mode` naming the subscription flow, and an `id_token` that is a syntactically valid JWT with an empty claim set. Every credential field in it is blank: the proxy builds the upstream header from the resolved subscription, so anything else would be dead weight the container has no business holding. Its `last_refresh` is stamped at container start rather than fixed, because a stale one sends Codex off to refresh against an auth host the platform does not intercept — which fails, and takes the CLI with it.

An existing file is left alone, for the same reason the state file is merged rather than replaced: it may be a real credential from the CLI's own login.
...
What does not run in holder mode is what has no subject. Skills and [init scripts](resource-definitions.md#initscript) scoped to an agent have no agent to scope to, and the trace hook has no agent CLI to register on -- nor, for agn, a trace id to hand it. The rule is that preparation determined by the environment runs, and preparation determined by the agent does not.
