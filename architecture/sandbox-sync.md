...
2. **Identity check.** The controller compares the sandbox root's marker against its session state, and the local root's inode against what it recorded. A missing or different marker, or a local root that is no longer the same directory, **halts before anything is read or written** — on whichever side failed.
...
| **Root identity** | Tracked on both sides, by different means. The sandbox root carries a marker file, because the platform created that directory and can write to it freely. The local root is identified by its **inode** alone, recorded at session creation and re-checked before every cycle: a directory deleted and recreated, an external drive unmounted, and a mountpoint that never mounted all present a different inode, and each would otherwise scan as an empty tree |
| **Why not the device number** | `st_dev` is assigned at mount time rather than being a property of the filesystem, so it changes across remounts on btrfs subvolumes, device-mapper, overlayfs, external drives, disk images, and most macOS non-boot volumes. Sessions are expected to survive reboots, so including it would halt resumed sessions on directories that never changed — the false positive that erodes the signal every other halt depends on. It buys only one case inode misses, a different filesystem mounted at the same path, and that case is caught anyway: filesystem roots share low fixed inode numbers, but a swapped volume presents the tracked entries as missing and trips the content-loss guard |
...
| A side that has lost most of its tracked content since the last cycle halts rather than propagating the loss | The load-bearing guard, not a supplement. The inode check is a cheap fast path for the common cases; this is what covers everything it cannot see — a partial unmount, a half-restored backup, an interrupted checkout, an inode reused after a delete-and-recreate, or a different filesystem mounted at the same path whose root inode collides |
