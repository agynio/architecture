...
> **Open question:** Per-tenant namespaces may be needed for stronger isolation (network policies, resource quotas per tenant). For now, a single namespace is sufficient. See [Open Questions](../open-questions.md).
...
Same as the general [Runner authentication](runner.md#authentication). The k8s-runner enrolls using a service token and authenticates all connections via OpenZiti mTLS. See [Authentication](authn.md#network-identity-openziti).
