...
- A public rule and a private resource claiming the same hostname are fine side by side, and broken only for an agent that ends up able to reach both — its traffic to that hostname then goes to one or the other unpredictably. Attaching or granting the second one directly is refused with both named, but membership in a group that holds one, or a change to which environment an agent runs, reaches the same state without either refusal firing. The platform reports those within a reconciliation pass, on both the rule and the resource, rather than pretending to prevent them.
