...
- **Skills** — instructions the agent loads when it needs them. Each skill has a name, a description of when the skill applies, and a body (text editor). The name is a slug, because it names the skill's directory on the agent's filesystem. The description is what the agent reads to decide whether to open the body, so the form asks for the trigger rather than a summary.
...
| A property with `format: password` | Text input drawn obscured, with a control to reveal it, so a token stays off a screen share. The value is sent to the browser like any other — the platform does not withhold it, and the form does not pretend otherwise |
