...
**Image detail** — name, type, repository, visibility, description, credential (the username and the name of the secret holding the password), and the discovered version list. Versions show tag, pushed date, and description; a stale image shows when discovery last succeeded. Actions: update name/description/credential/visibility/tag filter, refresh versions, delete. Repository and type are shown but cannot be changed.

**Register image** — name (required), type (required), repository (required), visibility (required), description, tag filter, and optionally a username and a credential. The credential is chosen from the organization's [secrets](../../architecture/secrets.md), with **add a new secret** offered in the same picker so creating one never means leaving the form; the password reaches the Secrets service and the image is given the id. The repository is read before the record is stored, so a wrong address or credential fails here rather than at workload start.
...
**Resource list** — table of resources across all of the organization's networks. Columns: name, network (link to network detail), protocol (`tcp` / `http` / `https`), intercept address (`intercept_host:ports`), target address (`target_host:ports`), grant count, egress rule (the rule naming this resource, linked, or empty), reachability (derived from the owning network's tunnels), provisioning state. Default sort: creation time, newest first.
...
**Resource detail** — its own page, not a card in a list. Shows the network it belongs to, protocol, target host and ports, intercept host and ports, provisioning state, and a **Copy connection string** affordance (`prod-postgres.corp:5432`) for pasting into agent configuration or tooling. An `http`/`https` resource also shows its egress rule: the rule naming it with its effect summary, or — when none exists — a **Create egress rule** action that opens the rule form with this resource preselected.
...
**Delete resource** — blocked while an egress rule names it; the dialog links to the rule and prompts the user to delete it first. Changing a resource's protocol to `tcp` is blocked the same way.
...
- **Destination** — for `Public`, a domain pattern (required; single-segment wildcards like `*.github.com`; reserved zones `*.agyn`, `*.svc`, `*.cluster.local`, and the `100.64.0.0/10` synthetic range are rejected inline, as is a hostname already claimed by a private resource) plus ports (defaults to `80, 443`). For `Private`, a resource picker listing the organization's `http` and `https` resources with their network and the hostname agents dial; `tcp` resources are shown disabled with the reason, and resources that already have a rule are excluded. Ports are not editable for a private destination — the resource's own ports are shown read-only. Each destination is unique per organization; the form rejects a duplicate with a clear message.
