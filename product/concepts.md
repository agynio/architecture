...
| **Environment** | An organization-level runtime definition: a runner, a flavor on that runner, a workspace image, optionally an agent runtime image — and the volumes, MCP servers, init scripts, environment variables, and egress rules a workload in it carries. Agents and sandboxes run in environments; running in one is a permission of its own. |
| **Volume** | A named mount declared on an environment (or on an MCP server): a path, whether it persists, and a size. One disk is provisioned per agent instance or sandbox running it; nothing is shared between them, and an environment that declares no volume has no persistent storage. |
