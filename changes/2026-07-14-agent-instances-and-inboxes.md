...
- **Supersedes and absorbs the 2026-04-12 Agent-to-Agent Communication change** (`2026-04-12-agent-to-agent-communication.md`, deleted in the same commit that introduced this file; retrievable from git history). Still-valid deltas from it — the nickname index, nickname registration across users/agents/apps, and the `agyn threads` command group — are carried in this file. Its passive-participant model (`--passive` flag, `Participant.passive` column, orchestrator passive skip) and `THREAD_ID`-scoped message processing are dropped entirely, replaced by instance participation and the inbox.
