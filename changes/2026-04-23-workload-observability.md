...
- `RunnersGateway.StreamWorkloadLogs` is exposed through the Gateway as a pass-through. The Runners service authorizes the caller as a member of the workload's organization, resolves the hosting runner from the workload record, and forwards the stream over OpenZiti.
