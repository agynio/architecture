...
- **Spec inconsistency to resolve as a prerequisite.** [Notifications — Room Naming](../architecture/notifications.md#room-naming-convention) describes `workload:{id}` rooms as accessible to org `member`s, while [Authorization — Notifications Service](../architecture/authz.md#notifications-service) requires `can_view_workloads` (owner-only). Subscribing the chat-app on behalf of non-owner participants requires the `member` reading. The two specs must be reconciled in favor of `member` before the chat-app subscription path is enabled. This is documentation-only — the subscription auth check itself lives in the Notifications gateway.
...
- The chat-app subscribes to `workload:{id}` rooms for every ID in `active_workload_ids` of currently-visible chats, and unsubscribes when an ID drops off the list (chat scrolls out of view, refresh removes the workload). The workload-room auth check in [Authorization — Notifications Service](../architecture/authz.md#notifications-service) reads as `member` on the workload's org (resolving the disagreement with [Notifications — Room Naming](../architecture/notifications.md#room-naming-convention) in favor of `member`).
...
The `workload:{id}` notification-room auth disagreement between [Notifications — Room Naming](../architecture/notifications.md#room-naming-convention) (`member`) and [Authorization — Notifications Service](../architecture/authz.md#notifications-service) (`can_view_workloads`) is a pre-existing inconsistency surfaced — not caused — by this change. Resolving it in favor of `member` is a prerequisite for the chat-app subscription path; the prerequisite is documentation-only when the Notifications gateway already enforces `member` (verify in code), or a one-line auth-code change when it currently enforces `can_view_workloads`. Either way, the resolution belongs to a separate change to the Notifications service spec and is not in scope here beyond flagging.
