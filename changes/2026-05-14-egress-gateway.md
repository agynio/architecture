...
- [Agents Orchestrator — Workload Network Policy](../architecture/agents-orchestrator.md#workload-network-policy)
- [Agents Orchestrator — Egress CA Distribution](../architecture/agents-orchestrator.md#egress-ca-distribution)
- [Runners — Cluster CIDR Configuration](../architecture/runners.md#cluster-cidr-configuration)
- [Runner — Inline Files](../architecture/runner.md#inline-files)
- [Runner — Network Policy](../architecture/runner.md#network-policy)
...
The new EgressRules service, the Egress Gateway, the workload NetworkPolicy installed by the Orchestrator, and the cert-manager-managed Egress CA collectively close this gap. The desired state is described in the documents linked above; this delta enumerates what's missing.
...
- **Per-rule OpenZiti service provisioning** — the EgressRules service must call [Ziti Management](../architecture/openziti.md) `CreateService` on rule create (with `intercept.v1` keyed on `matcher.domain_pattern` and `matcher.ports`, and `host.v1` with `forwardAddress: true`, `forwardPort: true`, `forwardProtocol: true`) and `DeleteService` on rule delete. Attaching a rule must call `CreateServicePolicy` (Dial, `identityRoles: ["#agent-<agent_id>"]`, `serviceRoles: ["@egress-rule-<rule_id>"]`); detaching must call `DeleteServicePolicy`. Following the [Expose Service](../architecture/expose-service.md) pattern. Reconciliation loop required to repair drift.
...
- **NetworkPolicy installed alongside each workload** — the [Agents Orchestrator](../architecture/agents-orchestrator.md) must include a NetworkPolicy in the workload spec the runner creates. The policy allows egress to the OpenZiti synthetic range (`100.64.0.0/10`), cluster DNS, and public internet; blocks the cluster pod CIDR, cluster service CIDR, and any operator-declared internal CIDRs. The [k8s-runner](../architecture/k8s-runner.md) must create the NetworkPolicy resource in addition to the Pod.

- **Runner CIDR configuration** — the [Runners](../architecture/runners.md) `Runner` resource gains three fields (`cluster_pod_cidr`, `cluster_service_cidr`, `additional_excluded_cidrs`). In-cluster k8s-runner auto-discovers them; external runners declare them at registration. The Orchestrator includes them in the workload spec so the runner constructs the NetworkPolicy correctly.
...
- An external runner hosting an agent pod receives the inline CA bytes and runner-CIDR config in the workload spec, materializes both locally (CA mount + NetworkPolicy in the runner's namespace), and the agent in that pod completes the GitHub test above end-to-end identically to an in-cluster agent. The external runner's cluster contains only K8s and the runner agent — no cert-manager, no trust-manager, no platform-specific webhook or operator.
...
- The workload NetworkPolicy is part of the same uniform contract: orchestrator constructs it from the runner's CIDR config and ships it as part of the workload spec. External runners install it in their local cluster the same way they install the Pod.
