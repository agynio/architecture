...
- SPA: Service Worker registration and token lifecycle (`postMessage` for JWT updates)
- SPA: Service Worker fetch interception for `media.agyn.dev` (auth header injection, `no-cors` → `cors` mode change)
- SPA: markdown renderer media components (`<img>`, `<video>`, `<audio>`) using proxy URLs
- SPA: download-original link on each rendered media element
...
- Agent responses containing `![](https://...)` render images inline in the UI.
